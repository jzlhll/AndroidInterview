...
https://www.jianshu.com/p/fdc12d049524 todo
...
1. 引用计数法：无法解决循环引用
2. 标记&清除算法：大量碎片，无法申请连续内存，也会导致加大回收频率。
3. 标记&压缩算法：其实是标记&清除的进阶，往一边赶。
4. 标记&复制算法：对半分，每次往一半用，用不够就左右拷贝；
5. 逐代回收：结合上述分出各种年代。Android没有。

andorid GC算法 android8前没有分代收集：

CMS 4.4~8.0：

初始化标记stw，并发标记，并发预处理，重新标记stw，并发清理。

其实就是并发版本的**标记-清理**。只有当没有连续内存，或者切换到后台的时候，才**压缩**。

很少压缩，已经有虚拟连续内存分配器。

但是android也有优化：划分了多个区域，相互拷贝。

Android8：

CC：并发复制。stop the world成为常量级别。

可达性分析(GCRoot关联)；判断内存已死：

​	 **虚拟机栈（≈函数）、JNI对象（native变量）、静态引用对象（static）、常量引用对象（static）**
...
appt2: R.java+code+aidl 

javac-> class 3rd class

=》dex

resource aapt =》 apk =》 signed apk =》 zipalign对齐 apk
...
循环里面还设计了屏障机制来保证系统View的调度消息优先执行，以及adleHandler的设计，最低优先级的执行。
