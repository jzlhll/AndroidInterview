...
>
> 一、ANR 产生的核心原因
> ANR（应用无响应）的本质是应用主线程（ActivityThread）被阻塞，无法在系统规定超时时间内完成指定操作；系统设计逻辑为执行组件启动、事件分发等操作时，会发送延时 ANR 消息，操作完成则移除消息，未完成则触发 ANR 并 dump 现场，具体分四类核心场景，且均与主线程阻塞直接相关：
>
> Service 超时 ANR：前台 Service 的 onCreate/onStartCommand 在 20s 内未完成，后台 Service 超时 200s，前台 Service 单独为 10s 超时阈值；
> 广播超时 ANR：前台广播的 onReceive 在 10s 内未完成，后台广播超时 60s；
>
> ContentProvider 超时 ANR：仅初始化阶段（onCreate）会触发，超时 10s；其增删改查操作，独立进程下走 Binder 线程、同进程下走调用线程，均不会引发 ANR；
>
> 输入事件分发超时 ANR：按键、触摸等输入事件在 5s 内未完成分发，该超时检测在 SystemServer 的 C++ 层执行，ViewRootImpl 未及时处理并移除事件队列的 ANR 消息则触发；
>
> 主线程阻塞的根源分为两类：一是 MessageQueue 的 next () 方法执行慢（如 UI 绘制耗时），二是 dispatchMsg () 处理消息过久（方法本身耗时、算法低效、CPU 被抢占、内存不足、IPC 超时等均会导致）。
>
> 二、ANR 快速定位
> 核心获取ANR 现场日志：系统触发 ANR 时会自动生成 traces.txt 文件（路径：/data/anr/），该文件记录了主线程阻塞的完整堆栈信息；adb report。
> 日志分析重点：定位主线程中耗时的具体方法、所属组件，区分 ANR 类型（Service / 广播 / ContentProvider / 输入事件）；
> 辅助排查：结合日志排查细分诱因，如是否存在 IPC 超时、CPU 占用过高（被其他进程抢占）、内存不足（频繁 GC）等问题。
>
> 三、ANR 通用修正方案
> 核心原则：所有耗时操作均移出主线程，保证主线程仅处理 UI 绘制、事件分发、轻量逻辑，针对性修正如下：
> 组件相关修正：Service 耗时逻辑改用 IntentService 或手动开启子线程；广播耗时操作改用异步广播，或在 onReceive 中启动 Service 处理；ContentProvider 的 onCreate 中避免耗时初始化，将逻辑移至子线程；
> 输入事件修正：优化 ViewRootImpl 的事件分发逻辑，避免在 onTouch、onClick 等输入事件回调中执行耗时操作；
> 通用耗时操作优化：将网络请求、文件 IO、数据库批量操作、复杂计算等耗时逻辑，通过线程池、协程等移至子线程，主线程仅接收结果并更新 UI；
> 其他诱因修正：优化低效算法减少执行时间；解决 IPC 超时问题（如简化跨进程通信逻辑）；优化内存（减少频繁 GC）；避免主线程做大量同步锁操作，防止 CPU 被抢占。
...
#### 全量 Android 标准 ANR 类型汇总表

|        ANR 类型        | 超时阈值 |          核心触发系统服务           |                       核心待处理任务                       |                         核心统一机制                         |
| :--------------------: | :------: | :---------------------------------: | :--------------------------------------------------------: | :----------------------------------------------------------: |
|      输入事件 ANR      |    5s    | Native 层 InputManagerService (IMS) |          点击 / 触摸 / 按键等<br />输入事件的处理          | 「发核心任务处理消息 + 对应阈值延迟 ANR 消息」，主线程阻塞→无法移除延迟消息→超时触发 ANR |
|      广播执行 ANR      |   10s    |          Framework 层 AMS           |  广播 Receiver#onReceive () <br />方法执行（强制主线程）   |                             同上                             |
| 前台服务启动（FGS）ANR |    5s    |          Framework 层 AMS           | 前台服务启动后 5s 内调用<br /> startForeground () 显示通知 |                             同上                             |
|    普通服务启动 ANR    |   20s    |          Framework 层 AMS           |     Service#onCreate/<br />onStartCommand/onBind 执行      |                             同上                             |
|  内容提供者（CP）ANR   |   20s    |        Framework 层 AMS+PMS         |      ContentProvider#onCreate 执行（应用初始化阶段）       |                             同上                             |
