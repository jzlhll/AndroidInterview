...
>  不要管为什么顶在statusBar上面，这是因为android15系统，默认让edgeToEdge沉浸式。后面将学到如何让它padding到合适位置。
...
- 支持触控以外的输入方式
 ...
 [构建自适应应用  | Jetpack Compose  | Android Developers](https://developer.android.google.cn/develop/ui/compose/build-adaptive-apps?hl=zh-cn)
...
| 数据采用State (remember + mutableStateOf/...)方式承载。 State/mutableStateOf ?前面看到了，该如何正确的做异步？似乎我们是有state来持有和更新数据，结合到flow，viewModel该如何做。 |
| flow如何结合？                                               |
| 几个避免该怎么做？避免更新共享数据和ViewModel                |
