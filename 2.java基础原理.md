...
内存可见性：(修改后缓存及时变更。因为极快的时候，cpu是改了，但是内存变量并没有快速更新到寄存器。)

***原子性***：volatile不保证i++操作的多线程下的执行。所以一般是bool的赋值或者变量的单例。

有序性：jvm保证单线程执行结果无误；但是多线程不保证。通过**内存屏障**，禁止指令重排。
...
内存可见性，**原子性**，有序性。使用**临界区**。保证上述3个特性。



* 单例，为什么需要双检测？

> 首先外层的null判断其实是为了减少使用synchronized，因此，外层不是临界区了。那么就会存在线程1正在分配单例对象，内部并没有完成初始化完成；而线程2由于外层判断不为null，使用了它就会导致错误。所以需要volatile来保证指令不重排，保证判断null的正确性。
