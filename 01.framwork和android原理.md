...
ART虚拟机初期存在“安装慢”和“内存占用高”两大弱点，Android 7.0 起 ART 引入**JIT（运行中即时编译）+AOT （闲时重编译）混合编译**与**热点收集**机制，进一步改进：针对高频执行的代码（热点代码）优化编译与内存分配策略，兼顾安装速度与运行效率，同时降低GC对应用性能的影响。
...
> 最佳实践；
>
> 传数据：**Bundle≤40KB，裸 Binder≤900KB，大数据分页 / 传路径**，嵌套 Bundle 也控总大小；
>
> 控线程：**并发数≤15，主线程不做同步调用，oneway 控 350KB 内**；
>
> 防崩溃：**必抓 TLE/RemoteException，加超时 / 死亡监听，捕获即降级**；
>
> 提性能：**减少调用次数，精简传输数据，缓存 Binder 代理对象**；
>
> 守规则：**敏感服务必鉴权，及时解绑定避泄漏，耗时操作移子线程**。
...
### ComponentActivity NonConfiguration 机制 与 onSaveInstanceState 的核心区别（极简总结）

二者均为 Android 配置变更时的**数据保活方案**，但设计目标、存储特性、使用场景完全不同，核心区别集中在**目标、存储、数据、触发、用途**5 个关键维度，具体如下：

1. **核心目标**：NonConfiguration 机制为**保存大对象 / 核心数据容器**（如 ViewModelStore），支撑 ViewModel 跨配置变更的持久化；onSaveInstanceState 为**保存轻量临时 UI 状态**（如控件值、页面简单参数），做基础的状态恢复。
2. **存储介质 & 生命周期**：NonConfiguration 是**内存级非配置存储区**（系统专属内存），仅进程存活时有效，进程销毁则数据丢失；onSaveInstanceState 是**序列化后存系统缓冲区 / 磁盘**，可跨进程 / 进程意外重启，生命周期更长。
3. **数据类型 & 大小**：NonConfiguration**无严格大小限制**，可存复杂对象（直接传引用，无需序列化）；onSaveInstanceState 依赖序列化（Parcelable/Serializable），受 Binder 传输**1MB-8KB 硬限制**，仅支持基础类型 / 轻量序列化对象。
4. **触发 & 生效场景**：NonConfiguration**仅配置变更时触发**（保存 / 恢复），Activity 正常 finish 不执行；onSaveInstanceState**配置变更 + 进程意外销毁**均触发，正常 finish 也不执行，是系统兜底的状态保存。
5. **使用主体**：NonConfiguration 是**框架层自用机制**（ViewModel 底层核心依赖），开发者几乎无需手动调用；onSaveInstanceState 是**开发者常用接口**，需手动重写保存 / 恢复 UI 相关轻量数据。

**一句话核心区分**：NonConfiguration 是**框架层为大对象 / 数据容器设计的内存级保活方案**，专为 ViewModel 服务；onSaveInstanceState 是**系统层为 UI 轻量状态设计的序列化保活方案**，为页面基础恢复兜底。
...
> Android 子 View 调用 `invalidate()` 后不直接绘制，核心是**脏区域传递**与**基于 VSYNC 信号的延迟绘制**，最终由 ViewRootImpl 触发三大核心流程，且仅绘制脏区域提升效率，整体流程精简为：
>
> 1. 子 View 计算自身脏区域，通过 `mParent.invalidateChild` 将脏区域交给父 View；
> 2. ViewGroup 循环向上传递脏区域，同时完成子→父坐标系转换，最终传递至真正的根 ViewRootImpl；
> 3. ViewRootImpl 合并脏区域，调用 `scheduleTraversals()` 排队绘制任务，先插入同步屏障屏蔽普通消息，再通过 Choreographer 注册下一帧 VSYNC 回调；
> 4. 系统 VSYNC 信号（每 16ms 一次）到来后，移除同步屏障恢复消息调度，执行 `performTraversals()` 触发三大流程；
> 5. 从根 View 开始递归执行 `measure` → `layout` → `draw`  **测量（计算宽高）、布局（确定位置）、绘制（仅绘脏区域）**，完成整个 UI 更新。
...
> 核心流程：
>
> **Activity 层（事件入口）**：系统将点击事件传给 Activity 的`dispatchTouchEvent`，直接转发给页面根 View（DecorView，本质是 ViewGroup），无额外判断。
>
> **ViewGroup 层（核心：逐层遍历 + 嵌套分发，最关键）**
>
> 这是**逐层向下传递**的核心，**每个父 ViewGroup 都会做相同的操作**，直到找到最终子 View：
>
> - 第一步：自身`onInterceptTouchEvent`判断是否拦截，拦截则自己处理（走`onTouchEvent`），不拦截则进入**子 View 遍历**；
> - 第二步：**按 Z 轴从顶到低遍历自身所有直接子 View**（叠在上面的子 View 先判断）；
> - 第三步：对每个子 View**实时判断是否命中**（点击坐标在子 View 区域内 + 子 View 可见 / 可触摸）；
> - 第四步：**命中则逐层向下分发**—— 将事件传给该子 View 的`dispatchTouchEvent`；若该子 View 还是 ViewGroup，就重复上述「判断拦截→遍历其子 View→命中分发」的动作，继续向下传递；**未命中则继续遍历下一个子 View**；
> - 第五步：若遍历完所有直接子 View 都未命中，当前 ViewGroup 自己处理事件。
>
> **View 层（最终消费判断）**：无子 View，直接判断 —— 命中区域 + View 可点击（如 Button、设置了 onClick 的 View），则执行`onTouchEvent`**消费事件**（返回 true），流程结束；否则**不消费**（返回 false），事件开始回传。
>
> **事件回传（未消费时）**：事件从最终 View**自下而上回传**，父 ViewGroup/View 依次判断是否消费，若都不消费，最终回传给 Activity，被系统抛弃。
...
当没有消息的时候，通过epoll/eventfd等方式，不阻塞线程但是阻塞住队列。当消息来了或者到点以后，唤醒队列，取得下一个消息。
