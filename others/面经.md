...
> Kotlin Flow 是基于协程的**冷数据流**，核心特性为**无收集则不执行**，仅当调用 collect () 收集时，上游的数据流逻辑才会启动，无收集操作时全程闲置。
>
> 采用生产者 - 消费者模型，生产者通过 emit () 发射数据，消费者通过 collect () 接收，生命周期与所在协程绑定。
>
> 依托协程挂起特性天然支持**背压**，能根据消费速度适配生产节奏，避免数据堆积，轻量实现异步流式数据处理。

协程的原理是什么？

> Kotlin 协程是**基于线程池的轻量级并发框架**，核心依托**suspend 挂起函数**和**状态机**实现。编译器将挂起函数编译为状态机，精准管理挂起点与恢复点，无线程上下文切换的高成本。通过**调度器（Dispatcher）** 完成协程到线程 / 线程池的灵活映射，挂起时释放占用线程，恢复时复用线程，非线程阻塞式执行，实现低开销的并发与异步操作。
...
1. mmap的本质：是内核虚拟内存与物理文件的映射，但用户层写内存≠直接写物理文件，而是先写入内核页缓存（Page Cache） —— 这是Linux系统的通用IO优化，系统会在空闲时异步懒刷盘，而非实时同步，目的是减少磁盘IO次数。
​
2. MMKV的默认策略：MMKV对mmap做了封装，默认写操作仅修改用户层的映射内存+内核页缓存，不主动触发刷盘； sync() （同步）/ async() （异步）的核心作用，就是主动告诉内核将页缓存中的数据刷写到物理文件，完成「内存-文件」的强同步。

补充：容易混淆的2个点

- ❌ 误区：mmap=内存和文件实时一致 → 实际是映射关系，而非实时同步关系，同步需要主动触发刷盘。
​
- ✅ 真相：未调用sync时，进程内读取是最新的（读的是映射内存），但跨进程/重启App后读取不到（读的是物理文件，未被更新），直到系统懒刷盘或主动调用sync。
