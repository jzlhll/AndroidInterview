...
PSI引擎是 AI 编程助手的**封闭式专属代码语义分析架构**，以自建私有抽象语法树 AST 为核心，通过代码解析、语义标注、查询检索三步完成代码理解，需独立维护代码模型、为单语言定制解析器，定制化程度高但生态耦合性强、集成成本高。

**12. LSP**

原生 LSP（Language Server Protocol）是 AI 编程助手的**开放式标准化代码服务架构**，通过统一协议实现编辑器与语言服务器解耦，依托标准接口、JSON-RPC 传输、增量同步机制，复用通用语言服务器实现多编辑器兼容、多语言覆盖与低耗高效代码服务，生态开放、可扩展性强。

设计逻辑： 定一套所有编辑器、所有语言都认的**通用沟通规则**，编辑器不用单独适配语言，语言也不用单独适配编辑器；代码变更只传改动部分，不用全量更新，直接对接 clangd、rust-analyzer 等现成语言服务器干活。

核心作用： **轻量、快、兼容广**—— 原生适配 VSCode/Neovim/ 浏览器 IDE 等，扩展简单，百万行代码库内存比 PSI 低 40%、操作延迟降 65%，大项目也流畅运行。
