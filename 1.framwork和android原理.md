...
>
> - ‌**基本数据类型**‌的值存储在栈上。
> - ‌**引用数据类型**‌（如对象）的实例存储在堆上，而引用本身（即指向对象的变量）存储在栈上。
...
* 数据限制
  ...
  ​	1MB左右。1MB-8K。ProcessState初始化时通过mmap向Binder驱动申请的内存空间大小。

  异步（oneway）事务的缓冲区限制更为严格，一半（约400KB）。这是因为异步事务不等待服务端处理完成，驱动需要预留一半的缓冲区用于同步事务的响应。

  ​	单个Bundle不能超过50KB，整个进程内所有bundle共享的内存大小不能超过1MB。
