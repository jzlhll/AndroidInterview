...
**核心原理**：RecyclerView 是高效列表控件，核心是**视图复用 + 按需加载**，减少 View 创建销毁，降低内存占用，解决 ListView 复用不灵活、布局单一问题。

**核心组成**：**LayoutManager**：负责 ItemView 测量、布局、复用，决定列表形式（线性、网格、瀑布流）；**Adapter**：绑定数据与视图，提供创建、绑定 ViewHolder 方法；**ViewHolder**：封装 ItemView 子 View 引用，避免重复 findViewById，是复用关键；**ItemDecoration/ItemAnimator**：负责分割线、动画。
